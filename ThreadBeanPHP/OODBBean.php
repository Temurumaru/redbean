...
use ThreadBeanPHP\Util\Either as Either;
 ...
 *
 * @file    ThreadBeanPHP/OODBBean.php
 * @author  Gabor de Mooij and the ThreadBeanPHP community
 * @license BSD/GPLv2
 * @desc    OODBBean represents a bean. ThreadBeanPHP uses beans
 *
 * @copyright
...
class OODBBean implements \IteratorAggregate,\ArrayAccess,\Countable,Jsonable
{
	 ...
	 * @param OODB    $redbean instance of the ThreadBeanPHP OODB class
				...
				if ( !$selected || isset( $selected[$key] ) ) {
					if ( is_array($value ) ) {
						if ( isset( $value['_type'] ) ) {
							$bean = $this->beanHelper->getToolbox()->getThreadBean()->dispense( $value['_type'] );
							unset( $value['_type'] );
							$bean->import($value);
							$this->$key = $bean;
						} else {
							...
							foreach( $value as $listKey => $listItem ) {
								$bean = $this->beanHelper->getToolbox()->getThreadBean()->dispense( $listItem['_type'] );
	...
	*/
	...
	/**
	 * Modifiers are a powerful concept in ThreadBeanPHP, they make it possible
	 * to change the way a property has to be loaded.
	 * ThreadBeanPHP uses property modifiers using a prefix notation like this:
	 *
	 * <code>
	 ...
	 * occurs. This is because ThreadBean allows you to query (probe) for
	...
	public function &__get( $property )
	{
		...
		if ( !$exists && !isset($this->$fieldLink) && (!$isOwn && !$isShared )) {
			 ...
			 * PHP 2. ThreadBeanPHP\OODBBean->__get() testje.php:22
	 ...
	 * ThreadBeanPHP as well as other systems how to deal with the bean.
	 ...
	 * </code>
	 *
	 * ThreadBeanPHP also stores meta data in beans, this meta data uses
	 ...
	 * a path-like notation for meta data in ThreadBeanPHP like:
	 ...
	 * determine whether you have made modifications to a list so ThreadBeanPHP
	...
	public function link( $typeOrBean, $qualification = array() )
	{
		if ( is_string( $typeOrBean ) ) {
			...
			$bean = $this->beanHelper->getToolBox()->getThreadBean()->dispense( $typeOrBean );
	 ...
	 * This is as far as support for 1-1 goes in ThreadBeanPHP. This
	 ...
	 * created (nor supported) by ThreadBeanPHP.
	 *
	...
	public function countOwn( $type )
	{
		...
		if ( $this->getID() ) {
			...
			if ( !self::$useFluidCount ) {
				...
				$count = $this->beanHelper->getToolbox()->getThreadBean()->count( $type, $sql, $bindings );
			}
		}
		$this->clearModifiers();
		return (int) $count;
	...
	public function &aggr( $list, $property, $type = NULL )
	{
		...
		$beans = $this->beanHelper->getToolBox()->getThreadBean()->batch( $type, $ids );
	 ...
	 * how to use equals() with ThreadBeanPHP-style enums.
	 *
