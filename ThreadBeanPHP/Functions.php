...
/**
 * Support functions for ThreadBeanPHP.
 * Additional convenience shortcut functions for ThreadBeanPHP.
 *
 * @file    ThreadBeanPHP/Functions.php
...
if (!function_exists('EID')) {
	...
	{
		return \ThreadBeanPHP\Facade::enum( $enumName )->id;
	}
...
if ( !function_exists( 'dmp' ) ) {
	...
	function dmp( $list )
	{
		print_r( \ThreadBeanPHP\Facade::dump( $list ) );
	}
}
...
if ( !function_exists( 'genslots' ) ) {
	...
	{
		return \ThreadBeanPHP\Facade::genSlots( $slots, $tpl );
	}
}
...
if ( !function_exists( 'array_flatten' ) ) {
	...
	{
		return \ThreadBeanPHP\Facade::flat( $array );
	}
}
...
if ( !function_exists( 'DBPrefix' ) ) {
	...
	function DBPrefix( $prefix = '\\Model' ) {
		return new \ThreadBeanPHP\BeanHelper\DynamicBeanHelper( $prefix );
	}
}
