...
namespace ThreadBeanPHP\QueryWriter;
...
/**
 * ThreadBeanPHP Abstract Query Writer.
 * Represents an abstract Database to ThreadBean
 * To write a driver for a different database for ThreadBean
 * Contains a number of functions all implementors can
 * inherit or override.
 *
 * @file    ThreadBeanPHP/QueryWriter/AQueryWriter.php
...
abstract class AQueryWriter
{
	...
	protected function check( $struct )
	{
		if ( !is_string( $struct ) || !preg_match( '/^[a-zA-Z0-9_]+$/', $struct ) ) {
			throw new RedException( 'Identifier does not conform to ThreadBeanPHP security policies.' );
		}
