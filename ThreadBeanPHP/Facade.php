...
use ThreadBeanPHP\QueryWriter as QueryWriter;
use ThreadBeanPHP\Adapter\DBAdapter as DBAdapter;
use ThreadBeanPHP\RedException\SQL as SQLException;
use ThreadBeanPHP\Logger as Logger;
use ThreadBeanPHP\Logger\RDefault as RDefault;
use ThreadBeanPHP\Logger\RDefault\Debug as Debug;
use ThreadBeanPHP\Adapter as Adapter;
use ThreadBeanPHP\QueryWriter\AQueryWriter as AQueryWriter;
use ThreadBeanPHP\RedException as RedException;
use ThreadBeanPHP\BeanHelper\SimpleFacadeBeanHelper as SimpleFacadeBeanHelper;
use ThreadBeanPHP\Driver\RPDO as RPDO;
use ThreadBeanPHP\Util\MultiLoader as MultiLoader;
use ThreadBeanPHP\Util\Transaction as Transaction;
use ThreadBeanPHP\Util\Dump as Dump;
use ThreadBeanPHP\Util\DispenseHelper as DispenseHelper;
use ThreadBeanPHP\Util\ArrayTool as ArrayTool;
use ThreadBeanPHP\Util\QuickExport as QuickExport;
use ThreadBeanPHP\Util\MatchUp as MatchUp;
use ThreadBeanPHP\Util\Look as Look;
use ThreadBeanPHP\Util\Diff as Diff;
use ThreadBeanPHP\Util\Tree as Tree;
use ThreadBeanPHP\Util\Feature;

/**
 * ThreadBean Facade
 *
 * Version Information
 * ThreadBean Version @version 5.7
 *
 * This class hides the object landscape of
 * ThreadBeanPHP behind a single letter class providing
 * almost all functionality with simple static calls.
 *
 * @file    ThreadBeanPHP/Facade.php
...
{
	/**
	 * ThreadBeanPHP version constant.
	 */
	...
	/**
	 * Returns the ThreadBeanPHP version string.
	 * The ThreadBeanPHP version string always has the same format "X.Y"
	 ...
	 * ThreadBeanPHP. The Setup() method can be called without any arguments, in this case it will
	...
	public static function addDatabase( $key, $dsn, $user = NULL, $pass = NULL, $frozen = FALSE, $partialBeans = FALSE, $options = array(), $beanHelper = NULL )
	{
		...
		if ( !is_null( $beanHelper ) ) {
			self::$toolboxes[$key]->getThreadBean()->setBeanHelper( $beanHelper );
		}
	}
	...
	public static function createToolbox( $dsn, $username = NULL, $password = NULL, $frozen = FALSE, $partialBeans = FALSE, $options = array() )
	{
		...
		$writerClass = '\\ThreadBeanPHP\\QueryWriter\\'.$writers[$wkey];
	...
	*/
	 ...
	 * with this bean and ThreadBean runs in fluid mode the schema
	 * will be altered to store the bean correctly.
	 * If the database schema is not compatible with this bean and
	 * ThreadBean runs in frozen mode it will throw an exception.
	 ...
	 * Hybrid mode is allowed (default OFF for novice), then ThreadBeanPHP
	 ...
	 * ThreadBeanPHP has only limited support for 1-1 relations.
	 ...
	 * ThreadBean uses the primary key ID $id and the string $type
	 * to find the bean. The $type specifies what kind of bean you
	 * are looking for; this is the same type as used with the
	 * dispense() function. If ThreadBean finds the bean it will return
	 * the OODB Bean object; if it cannot find the bean
	 * ThreadBean will return a new bean of type $type and with
	...
	/**
	 * Dispenses a new ThreadBean OODB Bean for use with
	 * the rest of the methods. ThreadBeanPHP thinks in beans, the bean is the
	 * primary way to interact with ThreadBeanPHP and the database managed by
	 * ThreadBeanPHP. To load, store and delete data from the database using ThreadBeanPHP
	 * you exchange these ThreadBeanPHP OODB Beans. The only exception to this rule
	 ...
	 * As with most Query tools in ThreadBean you can provide values to
	 ...
	 * having ThreadBeanPHP executing an SQL query you can use the fourth parameter to
	...
	/**
	 * Convenience function to fire an SQL query using the ThreadBeanPHP
	 ...
	 *
	 * * 'default' ThreadBeanPHP by default enforces Snake Case (i.e. book_id is_valid )
	 ...
	 *
	 * @warning ThreadBeanPHP transforms camelCase to snake_case using a slightly different
	...
	public static function configureFacadeWithToolbox( ToolBox $tb )
	{
		...
		self::$redbean            = self::$toolbox->getThreadBean();
	 ...
	 * </code>
	 *
	 * The example above illustrates how transactions in ThreadBeanPHP are used.
	...
	/**
	 * Generates and returns an ENUM value. This is how ThreadBeanPHP handles ENUMs.
	 ...
	 * the database adapter. It turns ThreadBeanPHP 'commands' into
	 * database 'statements'.
	 *
	 ...
	 * The ThreadBeanPHP Object oriented database is the main ThreadBeanPHP
	 * interface that allows you to store and retrieve ThreadBeanPHP
	 * objects (i.e. beans).
	 *
	 * @param OODB $redbean Object Database for facade to use
	 */
	public static function setThreadBean( OODB $redbean )
	{
	 ...
	 * @return OODB
	 */
	public static function getThreadBean()
	{
		return self::$redbean;
	}
	 ...
	 * scripts with a single ThreadBeanPHP command. Typically, matchUp() is used to
	 ...
	 *
	 * In previous versions of ThreadBeanPHP you had to use:
	...
	/**
	 * The gentleman's way to register a ThreadBeanPHP ToolBox instance
	...
	/**
	 * The gentleman's way to remove a ThreadBeanPHP ToolBox instance
	...
	/**
	 * Globally available service method for ThreadBeanPHP.
